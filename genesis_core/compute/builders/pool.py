...
class PoolBuilderService(sdk_builder.CollectionUniversalBuilderService):
    ...
    def _agent_by_pool(self, pool_uuid: sys_uuid.UUID) -> sys_uuid.UUID:
        ...
        raise ValueError("Pool %s not found", pool_uuid)
    ...
    def _can_create_machine(
        ...
        # pool = self._get_pool_from_iter_ctx(machine.pool)
    ...
    def _actualize_machine_derivatives_on_create_update(
        ...
        # NOTE(akremenetsky): Only for demo purposes
        # if volume.image == "http://10.20.0.1:8080/genesis-base1.raw":
        #     boot = machine.boot = nc.BootAlternative.hd0.value
        # else:
        #     boot = machine.boot = nc.BootAlternative.network.value
