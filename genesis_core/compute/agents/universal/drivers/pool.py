...
class MetaMachinePool(meta.MetaCoordinatorDataPlaneModel):
    ...
    dp_machine_map = properties.property(types.Dict(), default=lambda: {})
    dp_volume_map = properties.property(types.Dict(), default=lambda: {})
    ...
    def load_driver(self) -> driver_base.AbstractPoolDriver:
        ...
        class_ = utils.load_from_entry_point(
            nc.EP_MACHINE_POOL_DRIVERS, self.driver
        )
        driver = class_(self)
    ...
    def get_meta_model_fields(self) -> set[str] | None:
        ...
        """
        return {"uuid", "driver", "driver_spec", "machine_type"}
...
class MetaVolume(meta.MetaCoordinatorDataPlaneModel):
    ...
    path = types.AllowNone(types.String(max_length=255), default=None)
