...
class MetaMachinePool(meta.MetaCoordinatorDataPlaneModel):
    ...
    def restore_from_dp(self, **kwargs) -> None:
        ...
        self.dp_storage_pool_map = {
            p.uuid: p for p in driver.list_storage_pools()
        }

        # NOTE(akremenetsky): Use only the storage pool specified in the
        # driver spec.
        self.storage_pool_map = {
            str(p.uuid): p.dump_to_simple_view()
            for p in self.dp_storage_pool_map.values()
            if p.name == self.driver_spec["storage_pool"]
        }

        pool_info = driver.get_pool_info()
        self.all_cores = int(pool_info["cores"] * self.cores_ratio)
        self.all_ram = int(pool_info["ram"] * self.ram_ratio)
...
class MetaMachineVolume(meta.MetaCoordinatorDataPlaneModel):
    """Volume meta model."""
...
class PoolAgentDriver(meta.MetaCoordinatorAgentDriver):
    ...
    __model_map__ = {
        ...
        "machine_volume": MetaMachineVolume,
        "machine": MetaMachine,
        # "node": MetaNode,
        # "volume": MetaVolume,
    }
