...
class StoragePool(
    ...
    """The model represents a storage pool.

    The model is used to store information about the storage pool.
    So far it's only used for DP pool objects. In the future the `MachinePool`
    may refer to this model to store the pool information.
    """

    capacity = properties.property(types.Integer(min_value=0), default=0)
    available = properties.property(types.Integer(min_value=0), default=0)
    pool_type = properties.property(types.String(), required=True)

    # TODO(akremenetsky): Use additional dynamic properties to specify
    # the pool parameters. For instance the `dir` pool type may have
    # `path`, `permissions`, `owner`, `group` etc as additional properties.
...
class MachinePool(
    ...
    @property
    def storage_pools(self) -> tp.Collection[StoragePool]:
        return tuple(
            StoragePool.restore_from_simple_view(**p)
            for p in self.storage_pool_map.values()
        )
