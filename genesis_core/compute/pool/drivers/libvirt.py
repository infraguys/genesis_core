...
class LibvirtPoolDriver(base.AbstractPoolDriver):
    ...
    def attach_volume(self, volume: models.MachineVolume) -> None:
        ...
        # Detect pool path and absolute image path
        # pool_xml = minidom.parseString(storage_pool.XMLDesc())
        # pool_path = pool_xml.getElementsByTagName("path")[0].firstChild.data
        # image_path = f"{pool_path}/{vir_volume.path()}"
        image_path = vir_volume.path()

        # Map volume index to device name: 0 -> vda, 1 -> vdb, ...
        # if volume.index is None or volume.index < 0:
        #     raise ValueError("Volume index must be non-negative for attachment")
        # device_name = "vd" + chr(ord("a") + volume.index)
