...
class TestUsers(base.BaseIamResourceTest):
    ...
    @pytest.mark.parametrize(
        "login_by, grant_type",
        [
            ("username", c.GRANT_TYPE_PASSWORD_USERNAME),
            ("email", c.GRANT_TYPE_PASSWORD_EMAIL),
            ("login", c.GRANT_TYPE_PASSWORD_LOGIN),
        ],
    )
    def test_auth_case_insensitivity(
        self, login_by, grant_type, user_api, auth_test1_user
    ):
        params = {
            "username": "dummy_username",
            "password": auth_test1_user.password,
        ...
        params[login_by] = getattr(auth_test1_user, login_by).upper()
        auth = iam_clients.GenesisCoreAuth(**params)
        client = iam_clients.GenesisCoreTestRESTClient(
            f"{user_api.get_endpoint()}v1/",
            auth,
