...
class CoreAgentService(looper_basic.BasicService):
    ...
    def _actualize_render(self, render: models.Render) -> None:
        if render.does_exist_and_valid():
            return
    ...
    def _actualize_renders(
        ...
        for r in set(cp_payload.renders) & set(dp_payload.renders):
            for render in cp_payload.renders:
                if r.uuid == render.uuid:
                    break
            else:
                raise ValueError("Render not found in payload")

            try:
                self._actualize_render(render)
            except Exception:
                LOG.exception("Error actualizing render %s", render.uuid)
