...
class SecretServiceBuilder(basic.BasicService):
    ...
    def _actualize_changed_certificates(self) -> None:
        ...
        self._actualize_changed_secrets(sc.PASSWORD_KIND, changed_certs)
    ...
    def _actualize_outdated_certificate(
        ...
        saved = models.Certificate.from_ua_resource(actual_resource)

        # Actualize password
        if (
            saved.status != certificate.status
            or saved.key != certificate.key
            or saved.cert != certificate.cert
        ):
            certificate.status = saved.status
            certificate.key = saved.key
            certificate.cert = saved.cert
            certificate.save()
        ...
        if (
            ...
            # TODO(akremenetsky): The same issue like for configs. We cannot
            # set "ACTIVE" status if hashes are different
            target_resource.status = actual_resource.status
            target_resource.full_hash = actual_resource.full_hash
    ...
    def _actualize_certificates(self):
        try:
