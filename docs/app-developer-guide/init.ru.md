...
| Тип | Описание |
|---|---|
| **Python** | Python-приложение. Поддерживает менеджеры пакетов `pip` и `uv`. Опционально управляет сервисами systemd; PostgreSQL включается флагом `--enable-pgsql`. |
...
- **GitHub Actions** — ещё не реализовано: опция принимается, но файлы не генерируются.
